...
export class TopicProducer {
     ...
     *
     * @param inner The native node module created by `new Fluvio().connect().topicProducer()`
     * @param options Top-level Fluvio client options
     */
    constructor(inner: TopicProducer, options?: Partial<Options>) {
        this.inner = inner
    }
...
export interface PartitionConsumer {
    ...
    stream(offset: Offset, cb: Function): Promise<void>
}
...
export class PartitionConsumer {
     ...
     * @param options Top-level Fluvio client options
     */
    constructor(inner: PartitionConsumer, options?: Partial<Options>) {
        this.inner = inner
    }
    ...
    async stream(offset: Offset, cb: Function): Promise<void> {
        const event = new EventEmitter()
        event.on('data', (event: { batches: Batch[] }) => {
...
export interface FluvioAdmin {
    ...
    findTopic(topic: string): Promise<string>
...
export class FluvioAdmin {
     ...
     *
     */
    async findTopic(topic: string): Promise<string> {
...
export default class Fluvio implements FluvioClient {
    ...
    async topicProducer(topic: string): Promise<TopicProducer> {
        ...
        let inner = await this.client?.topicProducer(topic)
        if (!inner) {
            throw new Error('Failed to create topic producer')
        }
        return new TopicProducer(inner, this.options)
    }
    ...
    async partitionConsumer(
        ...
        this.checkConnection()
        let inner = await this.client?.partitionConsumer(topic, partition)
        if (!inner) {
            throw new Error('Failed to create partition consumer')
        }
        return new PartitionConsumer(inner, this.options)
    }
    ...
    async admin(): Promise<FluvioAdmin> {
        this.checkConnection()
        let inner = await this.client?.admin()
        if (!inner) {
